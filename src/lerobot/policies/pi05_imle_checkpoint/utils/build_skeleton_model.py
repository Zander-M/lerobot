...
from lerobot.policies.pi05_imle_checkpoint import PI05IMLECheckpointConfig, PI05IMLECheckpointPolicy
...
def build_checkpoint(pretrained_path: str, output_path: str) -> None:
    ...
    config = PI05IMLECheckpointConfig(**cfg_dict)

    policy = PI05IMLECheckpointPolicy.from_pi05(pretrained_ckpt, 
                                                config=config, 
                                               )
    policy.save_pretrained(output_dir)
    print(f"IMLE checkpoint saved to {output_path}")
    
    # Move pre/post Processors to the checkpoint folder
